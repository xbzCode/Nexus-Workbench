...
import type { MatchResult, TaskCreate, DescribeNodeResponse, DescribeWorkflowResponse, SSEEvent } from "@/lib/types";
...
export default function ChatPage() {
    ...
    for (const evt of sseEvents) {
      ...
      switch (evt.event) {
        case "dag:validation_passed":
          content = `DAG 校验通过，共 ${data.node_count} 个节点`;
          break;
        case "dag:topo_sorted":
          content = "拓扑排序完成，准备执行";
          break;
        case "dag:level_started":
          content = `开始执行第 ${data.level} 层`;
          break;
        case "dag:node_started":
          content = `节点 ${data.node_id} 开始执行`;
          break;
        case "node:thinking":
          content = `${data.node_id}: ${data.content}`;
          eventLabel = "thinking";
          break;
        case "node:progress":
          content = `${data.node_id}: ${data.content}`;
        ...
        case "dag:node_skipped":
          content = `节点 ${data.node_id} 跳过: ${data.reason}`;
          break;
        case "dag:level_completed":
          content = `第 ${data.level} 层执行完成`;
          break;
        ...
        case "approval:created":
          content = `需要审批: ${data.title}`;
          eventLabel = "approval";
          break;
        case "approval:resolved":
          content = `审批已处理: ${data.status}`;
          break;
        default:
  ...
  const handleSubmit = useCallback(async (text: string) => {
    ...
    if (text.startsWith("/workflow ")) {
      ...
      if (result) {
        setDescribeWorkflowResult(result);
      } else {
        setStep("input");
      }
      return;
    ...
    if (result) {
      ...
      setMatchResult({
        mode: "bare_agent",
        reasoning: "匹配服务暂不可用，将使用裸 Agent 模式",
      });
      setStep("result");
    }
  ...
  const handleConfirm = useCallback(async () => {
    ...
    const taskData: TaskCreate = {
      title: input.trim(),
      input_data: { user_input: input.trim() },
    };

    if (matchResult.mode === "matched") {
      taskData.workflow_id = matchResult.workflow_id ?? null;
    } else if (matchResult.mode === "dynamic_assembly") {
      taskData.execution_mode = "dynamic_assembly";
      taskData.dag = matchResult.dag ?? null;
    }

    const task = await createAndStart(taskData);

    if (task) {
      // 方案A：不跳转，进入 executing 状态，在 Chat 区域展示实时 SSE 事件
      setTaskId(task.id);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12">
        ...
        <div className="mt-8 w-full max-w-[640px]">
          <div className="relative">
              ...
              <button
                className={cn(
                  ...
                  input.trim() && step === "input"
                    ? "bg-brand text-brand-foreground shadow-md hover:opacity-90 active:scale-95"
            ...
            <div className="mt-2.5 flex items-center justify-center gap-3 text-[11px] text-muted-foreground/60">
              ...
              {step === "describing_node" && !describeNodeResult && <span>正在生成节点定义...</span>}
              {step === "describing_workflow" && !describeWorkflowResult && <span>正在生成工作流...</span>}
              {step === "input" && (
                <>
                  <span>
                    输入 <kbd className="rounded border border-border bg-muted px-1 py-0.5 font-mono text-[10px]">/</kbd> 查看命令
                  </span>
                  <span>
                    按 <kbd className="rounded border border-border bg-muted px-1 py-0.5 font-mono text-[10px]">Enter</kbd> 发送
                  </span>
                </>
              )}
              {step === "result" && (
                <span>
                  按 <kbd className="rounded border border-border bg-muted px-1 py-0.5 font-mono text-[10px]">Esc</kbd> 取消
                </span>
              )}
            </div>
          </div>
        ...
        {/* Execution live log — 方案A：任务创建后在 Chat 区域展示实时进度 */}
        {step === "executing" && taskId && (
          <div className="animate-slide-up mt-6 w-full max-w-[640px]">
            <div className="rounded-2xl border border-border bg-card shadow-lg overflow-hidden">
              {/* Header */}
              <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-surface-hover/50">
                <div className="flex items-center gap-2.5">
                  {taskCompleted ? (
                    <CheckCircle2 className="h-4.5 w-4.5 text-emerald-500" />
                  ) : (
                    <Loader2 className="h-4.5 w-4.5 text-brand animate-spin" />
                  )}
                  <span className="text-sm font-medium text-foreground">
                    {taskCompleted ? "执行完成" : "正在执行..."}
                  </span>
                  {matchResult && (
                    <span className="text-xs text-muted-foreground">
                      {matchResult.mode === "matched"
                        ? matchResult.workflow_name
                        : matchResult.mode === "dynamic_assembly"
                        ? "动态工作流"
                        : "裸 Agent"}
                    </span>
                  )}
              ...
              {/* Log entries */}
              <div className="max-h-[320px] overflow-y-auto px-5 py-3 space-y-1.5">
                ...
                {executionLogs.map((log) => (
                  <div
                    key={log.id}
                    className={cn(
                      "flex items-start gap-2 text-[13px] leading-relaxed py-0.5",
                      log.event === "thinking" && "text-violet-400",
                      log.event === "progress" && "text-muted-foreground",
                      log.event === "question" && "text-amber-500",
                      log.event === "approval" && "text-amber-500",
                  ...
                  >
                    <span className="shrink-0 mt-0.5">
                      ...
                      ) : log.event === "dag:node_completed" ? (
                        <CheckCircle2 className="h-3.5 w-3.5" />
                      ) : log.event === "dag:node_failed" ? (
                        <XCircle className="h-3.5 w-3.5" />
                      ) : log.event === "question" || log.event === "approval" ? (
                        <AlertCircle className="h-3.5 w-3.5" />
                      ) : log.event === "dag:execution_completed" ? (
              ...
              {taskCompleted && (
                <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-border bg-surface-hover/30">
                  ...
                  <button
                    className="flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-brand-foreground hover:opacity-90 transition-all"
                    onClick={() => router.push(`/tasks/${taskId}`)}
                  >
                    <ExternalLink className="h-3.5 w-3.5" />
                    查看详情
        ...
        <div
          className={cn(
            "mt-8 grid w-full max-w-[640px] grid-cols-2 gap-3 sm:grid-cols-4 transition-all",
          ...
          {QUICK_EXAMPLES.map((ex) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[13px] leading-snug text-foreground group-hover:text-brand transition-colors">
                  {ex.text}
                </span>
