...
├── models/        ← SQLAlchemy 2.0 ORM（mapped_column），按实体拆文件
├── schemas/       ← Pydantic V2 请求/响应 schema，与 model 分离
├── core/          ← 核心引擎（与业务无关，可独立测试）
│   ├── dag/       ← DAG 模型 + 校验 + 序列化（JSONB ↔ DAGDefinition）
│   ├── scheduler/ ← 拓扑排序 + AST安全条件求值 + 数据流传递
│   ├── executor/  ← 执行引擎（Mock/Adapter双模式 + 审批暂停/恢复）
...
├── services/      ← 业务逻辑层（每个实体一个 service 文件）
└── api/           ← REST 路由层（FastAPI 路由，DI 通过 deps.py）
```
...
- **App Router** 页面：首页（匹配/描述）、workflows（DAG 可视化）、tasks（执行列表+详情）、approvals（审批队列）、nodes（节点管理）
...
- PostgreSQL 16 + Alembic 迁移，11 张表（user/workflow/node/task/approval/execution_path/snapshot 等）
